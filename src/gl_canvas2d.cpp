...
void CV::polygon(Vector2d points[], int n_elems)
{
   int cont;
   glBegin(GL_LINE_LOOP);
   for (cont = 0; cont < n_elems; cont++)
   {
      glVertex2d(points[cont].x, points[cont].y);
   }
   glEnd();
}
...
void CV::polygonFill(Vector2d points[], int elems)
{
   int cont;
   glBegin(GL_POLYGON);
   for (cont = 0; cont < elems; cont++)
...
void CV::circleFill(Vector2d vet, float radius, int div)
{
   CV::circleFill(vet.x, vet.y, radius, div);
}
...
void CV::focusWindows(Vector2i vec)
{
   glutPositionWindow(vec.x, currentScreenHeight - vec.y);
}
